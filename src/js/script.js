$(document).ready(function () {
    ...
    const images = [
        "./src/images/filter_system.jpg",
        "./src/images/ecosoft_green.jpg",
        "./src/images/filter_one.jpg"
    ];

    const imageContainerMobile = document.querySelector(".slider-image-mobile");
    const imageContainerDesktop = document.querySelector(".slider-image-desktop");

    let currentIndex = 0;

    function changeImage() {
        const currentImage = images[currentIndex];
        imageContainerMobile.src = currentImage;
        imageContainerDesktop.src = currentImage;
    ...
    //--company offers variety start--
    //buttons "Читати детальніше"/ "Згорнути":
    ...
    // Визначення поточної мови на основі атрибуту lang
    const currentLanguage = document.documentElement.lang;

    // Функція для зміни тексту кнопки залежно від мови
    function updateButtonText() {
        const isExpanded = readMoreButton.getAttribute("data-expanded") === "true";
        const newText = isExpanded ? languageResources[currentLanguage].collapse : languageResources[currentLanguage].readMore;

        readMoreButton.setAttribute("data-expanded", isExpanded ? "false" : "true");
        readMoreButton.textContent = newText;
    }

    readMoreButton.addEventListener("click", updateButtonText);

    // Ініціалізація тексту кнопки при завантаженні сторінки
    updateButtonText();
    //--company offers variety end--
    ...
    //modal window end
    //loader start
    let mask = document.getElementById("loader");
